...
Phase 2E-FINAL Stage 5B-4B-2 adds receipt gates for bond lock / `PendingObservation` and approve certification. The strict bond lock path is `lock_green_label_bond_with_fee_receipt_v1`; the legacy no-receipt `lock_green_label_bond` now fails closed. See `docs/green-label-certification-fee-receipt-gates-v1.md`.
