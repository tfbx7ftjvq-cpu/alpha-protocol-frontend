-- Alpha Protocol Phase 2E-6B-4K
-- Supabase Solana Web3 identity subject compatibility.
--
-- A live Staging sign-in confirmed that Supabase stores the canonical
-- identity as `web3:solana:<base58-address>` in both provider_id and
-- identity_data.sub. The earlier migration expected separate chain/address
-- properties and therefore failed closed. This migration replaces only the
-- identity resolver. It does not change or activate the intake control row.
