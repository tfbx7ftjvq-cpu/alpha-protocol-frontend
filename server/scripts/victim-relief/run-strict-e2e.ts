...
function originalPayoutTx(ctxIdl: any): Transaction {
  ...
  const ix = buildIx(ctxIdl, "execute_victim_relief_approved_payout_v1", [
    ...
    meta(c.config),
    meta(c.policy),
    meta(c.claimantState, false, true),
    meta(c.caseKey, false, true),
    meta(c.snapshot),
    meta(c.request, false, true),
    meta(deriveDecisionRecord(g.queue)),
    meta(treasury.treasuryConfigV2),
    meta(treasury.vaultAuthorityV2),
    meta(treasury.reliefUsdcVault, false, true),
    meta(c.recipient, false, true),
    meta(DEVNET_USDC_MINT),
    meta(derivePayoutRecord(c.request), false, true),
    meta(pk(process.env.EXECUTOR || process.env.ANCHOR_WALLET_PUBKEY, "EXECUTOR or ANCHOR_WALLET_PUBKEY"), true, true),
    meta(TOKEN),
    meta(SYS),
  ]);
...
function rejectUpholdTx(ctxIdl: any): Transaction {
  ...
  const ix = buildIx(ctxIdl, "execute_uphold_victim_relief_appeal_v1", [
    meta(deriveGovernanceConfig()),
    meta(deriveProtocolModuleRegistry(MODULE_CODES.VictimRelief)),
    meta(g.proposal),
    meta(g.action),
    meta(g.adapter),
    meta(g.decision),
    ...
    meta(c.config),
    meta(c.caseKey, false, true),
    meta(c.appeal, false, true),
    meta(c.policy),
    meta(c.snapshot),
    meta(deriveDecisionRecord(original.queue)),
    meta(treasury.treasuryConfigV2),
    meta(treasury.vaultAuthorityV2),
    meta(treasury.reliefUsdcVault),
    meta(DEVNET_USDC_MINT),
    meta(deriveAppealRecord(g.queue), false, true),
    meta(pk(process.env.EXECUTOR || process.env.ANCHOR_WALLET_PUBKEY, "EXECUTOR or ANCHOR_WALLET_PUBKEY"), true, true),
    meta(SYS),
  ]);
  return new Transaction().add(ix);
...
function overturnPayoutTx(ctxIdl: any): Transaction {
  ...
  const ix = buildIx(ctxIdl, "execute_victim_relief_overturn_payout_v1", [
    ...
    meta(c.snapshot),
    meta(deriveDecisionRecord(original.queue)),
    meta(deriveAppealRecord(g.queue)),
    meta(c.request, false, true),
    meta(treasury.treasuryConfigV2),
    meta(treasury.vaultAuthorityV2),
    meta(treasury.reliefUsdcVault, false, true),
...
function cancelOriginalTx(ctxIdl: any): Transaction {
  ...
  const ix = buildIx(ctxIdl, "execute_cancel_original_victim_relief_payout_v1", [
    meta(deriveGovernanceConfig()),
    meta(deriveProtocolModuleRegistry(MODULE_CODES.VictimRelief)),
    meta(original.proposal),
    meta(original.action),
    meta(original.decision),
    meta(original.queue),
    meta(deriveDecisionRecord(original.queue)),
    meta(cancel.proposal),
    meta(cancel.action),
    meta(cancel.adapter),
    meta(cancel.decision),
    meta(cancel.queue),
    meta(c.config),
    meta(c.policy),
    meta(c.claimantState, false, true),
    ...
    meta(c.request, false, true),
    meta(treasury.treasuryConfigV2),
    meta(derivePayoutCancelRecord(c.request), false, true),
    meta(pk(process.env.EXECUTOR || process.env.ANCHOR_WALLET_PUBKEY, "EXECUTOR or ANCHOR_WALLET_PUBKEY"), true, true),
    meta(SYS),
  ]);
  return new Transaction().add(ix);
...
function cancelOverturnTx(ctxIdl: any): Transaction {
  ...
  const ix = buildIx(ctxIdl, "execute_cancel_overturn_victim_relief_payout_v1", [
    meta(deriveGovernanceConfig()),
    meta(deriveProtocolModuleRegistry(MODULE_CODES.VictimRelief)),
    meta(overturn.proposal),
    meta(overturn.action),
    meta(overturn.decision),
    meta(overturn.queue),
    meta(c.appeal),
    meta(c.snapshot),
    meta(deriveDecisionRecord(original.queue)),
    meta(deriveAppealRecord(overturn.queue)),
    meta(cancel.proposal),
    meta(cancel.action),
    meta(cancel.adapter),
    meta(cancel.decision),
...
function pauseTx(ctxIdl: any): { label: string; tx: Transaction } {
  ...
  if (mode === "guardian") {
    ...
    const ix = buildIx(ctxIdl, "guardian_pause_victim_relief_v1", [
      ...
      meta(pk(process.env.EMERGENCY_GUARDIAN, "EMERGENCY_GUARDIAN"), true),
    ]);
  ...
  const ix = buildIx(ctxIdl, instructionName, [
    ...
    meta(g.queue),
    meta(config, false, true),
    meta(derivePauseRecord(g.queue), false, true),
    meta(pk(process.env.EXECUTOR || process.env.ANCHOR_WALLET_PUBKEY, "EXECUTOR or ANCHOR_WALLET_PUBKEY"), true, true),
    meta(SYS),
  ]);
