  ...
  if (!(await accountExists(connection, victimReliefConfig))) {
    tx.add(
      buildIx(ctx.idl, "initialize_victim_relief_config_v1", [
        meta("victim_relief_config", victimReliefConfig, false, true),
        meta("security_governance_config", governanceConfig),
        meta("treasury_config", treasury.treasuryConfigV2),
        meta("usdc_mint", DEVNET_USDC_MINT),
        meta("payer", payer, true, true),
        meta("bootstrap_authority", payer, true),
  ...
  if (!(await accountExists(connection, victimReliefPolicy))) {
    tx.add(
      buildIx(
        ...
        [
          meta("victim_relief_config", victimReliefConfig, false, true),
          meta("victim_relief_policy", victimReliefPolicy, false, true),
          meta("payer", payer, true, true),
          meta("authority", payer, true),
          meta("system_program", SYS),
        ],
        buildPolicyPayload(),
      ),
    ...
    tx.add(
      buildIx(
        ...
        [
          ...
          meta("bootstrap_authority", payer, true),
          meta("security_governance_config", governanceConfig),
          meta("protocol_module_registry", registry, false, true),
          meta("system_program", SYS),
        ],
