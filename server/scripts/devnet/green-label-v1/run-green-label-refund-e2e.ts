...
import {
  ...
  buildExecuteGreenLabelRefundIx,
...
async function main(): Promise<void> {
  printDevnetRiskBanner("run-green-label-refund-e2e");
  ...
  const actionType = readEnumEnv("ACTION_TYPE", "GreenLabelRefund", ACTION_TYPES);
  const proposalType = readEnumEnv("PROPOSAL_TYPE", "GreenLabelRefund", PROPOSAL_TYPES);
  const decision = readEnumEnv("DECISION", "Approved", PROPOSAL_DECISIONS);
  if (actionType.name !== "GreenLabelRefund" || proposalType.name !== "GreenLabelRefund") {
    throw new Error("Refund E2E requires ACTION_TYPE=GreenLabelRefund and PROPOSAL_TYPE=GreenLabelRefund.");
  }
  const payloadHash = readHashEnv(
    "PAYLOAD_HASH",
    `green-label-refund:${projectId.toString()}:${disputeId.toString()}`,
  );
  ...
  const treasuryBefore = await getTokenBalance(provider, config.baseBondTreasuryVault);
  ...
  await sendAndConfirmLabeled(
    provider,
    "create_security_refund_decision",
    new Transaction().add(
      buildCreateProposalDecisionIx({
        proposalId,
        proposalType,
  ...
  await sendAndConfirmLabeled(
    ...
    new Transaction().add(
      buildExecuteGreenLabelRefundIx({
        ...
        executor: wallet,
        projectOwnerUsdcAta,
        baseBondTreasuryVault: config.baseBondTreasuryVault,
        usdcMint: config.usdcMint,
        proposalId,
      }),
    ),
  ...
  const treasuryAfter = await getTokenBalance(provider, config.baseBondTreasuryVault);
  ...
  console.log("  base_bond_treasury_vault:", formatUsdc(treasuryAfter - treasuryBefore));
  ...
  await printVaultBalance(provider, "final base_bond_treasury_vault", config.baseBondTreasuryVault);
}

main().catch((error) => {
  console.error("run-green-label-refund-e2e failed:");
