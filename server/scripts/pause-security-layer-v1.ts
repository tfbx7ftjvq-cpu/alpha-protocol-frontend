...
async function main(): Promise<void> {
  ...
  const discriminator = anchorDiscriminator("pause_security_layer");
  console.log("pause_security_layer discriminator hex:", discriminator.toString("hex"));

  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const emergencyGuardian = provider.wallet.publicKey;
  ...
  const instruction = new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: emergencyGuardian, isSigner: true, isWritable: false },
    ],
    data: discriminator,
  });
